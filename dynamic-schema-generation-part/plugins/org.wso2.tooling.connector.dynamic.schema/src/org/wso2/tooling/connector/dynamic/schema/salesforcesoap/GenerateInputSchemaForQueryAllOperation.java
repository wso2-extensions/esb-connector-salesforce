...
public class GenerateInputSchemaForQueryAllOperation extends Dialog {
	...
	private Text queryAllTextField;
	...
	public GenerateInputSchemaForQueryAllOperation(Shell parent) {
		super(parent);
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		lblQueryAll = new Label(grpPropertyKey, SWT.NORMAL);
		...
		queryAllTextField = new Text(grpPropertyKey, SWT.MULTI | SWT.BORDER | SWT.WRAP | SWT.V_SCROLL);
		...
		cmbSObjectType.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent arg0) {
				try {
					queryAllTextField.setText(buildQuery());
				} catch (Exception e) {
					MessageDialog.openWarning(PlatformUI.getWorkbench().getDisplay().getActiveShell(),
		...
		FormData queryAllLabelLayoutData = new FormData();
		queryAllLabelLayoutData.top = new FormAttachment(lblSObject, 20, SWT.BOTTOM);
		lblQueryAll.setText(SELECT_SALESFORCE_QUERY);
		lblQueryAll.setLayoutData(queryAllLabelLayoutData);

		FormData queryAllTextFieldLayoutData = new FormData();
		queryAllTextFieldLayoutData.top = new FormAttachment(lblQueryAll, 15, SWT.BOTTOM);
		queryAllTextFieldLayoutData.left = new FormAttachment(0, 5);
		queryAllTextFieldLayoutData.right = new FormAttachment(100, -5);
		queryAllTextField.setLayoutData(queryAllTextFieldLayoutData);
		queryAllTextFieldLayoutData.height = 125;
	...
	/**
	 * Call QueryAll operation of Salesforce SOAP api to get the response to
	 * generate the Schema.
	 * 
	 * @return response.
	 * @throws Exception
	 */
	public String callQueryAll() throws Exception {
		SOAPConnectionFactory soapConnectionFactory = SOAPConnectionFactory.newInstance();
		SOAPConnection soapConnection = soapConnectionFactory.createConnection();
		...
		// SOAP Body
		SOAPBody buildSoapBody = envelope.getBody();
		SOAPElement soapBodyElem = buildSoapBody.addChildElement("queryAll", "urn");
		SOAPElement soapBodyElem1 = soapBodyElem.addChildElement("queryString", "urn");
		soapBodyElem1.addTextNode(queryAllTextField.getText());
